        ...
        if (req.getPathInfo()==null || req.getPathInfo().equals("/")){
            ...
            if (matcher.find()){
                ...
                if (!userId.equals(SecurityContextHolder.getPrincipal().getId())){
                    ...
                    jsonb.toJson(new HttpResponseErrorMsg(new Date().getTime(),403,null,"Permission denied",
                            req.getRequestURI()),resp.getWriter());
                    return;
                }
            }
            ...
            if(req.getPathInfo().matches("/[A-Fa-f0-9\\-]{36}/?")){
                getServletContext().getNamedDispatcher("UserServlet").forward(req,resp);
            } else if (req.getPathInfo().matches("/([A-Fa-f1-9\\-]{36})/lists(/\\d+)?/?")) {
